...
| `scripts/gen.py` | 一键生成 10 种版式（封面/信息图/金句卡/对比/步骤/数据卡/时间线/图文/图表/流程） | 写 SVG 之前 |
...
**任意比例/尺寸**：手写 SVG 直接改 `width/height/viewBox`；用 `gen.py` 生成则加 `--size 宽x高`（精确）或 `--aspect 宽:高 --width 宽`（按比例），十种版式都支持。常用平台尺寸速查：`python3 scripts/gen.py sizes`（公众号封面 / 小红书 3:4 / OG 1.91:1 / Banner 16:9 / 竖版海报 9:16 等）。
...
> **更快的方式**：`scripts/gen.py` 一键生成（套配色卡 + 自动换行/居中/网格 + 可选校验/渲染）。**10 种版式**：`cover`（封面，`--align left/center`）/ `infographic`（信息图，`--cols 1/2`）/ `quote`（金句卡）/ `compare`（左右对比）/ `steps`（横向步骤）/ `stats`（数据卡，`--cols N`）/ `timeline`（时间线）/ `feature`（两栏图文）/ `chart`（柱状+环形图）/ `flow`（流程图）。深底版式支持 `--deco dots/circles/grid/diag` 背景底纹，每种都支持 `--size`/`--aspect` 定尺寸，用 `python3 scripts/gen.py <类型> --help` 看参数。
...
内置 54 张经过对比度校验的专业配色卡（`scripts/palette.py`，全部通过 `check` 校验）：
...
| 19 | 春日樱 | 春日 / 少女 / 浪漫 | `#3A1A2E` / `#EC4899` / `#F472B6` |
| 20 | 夏日海 | 夏日 / 海洋 / 清凉 | `#0A3D4B` / `#06B6D4` / `#0EA5E9` |
| 21 | 秋日枫 | 秋日 / 丰收 / 温暖 | `#3B2013` / `#EA580C` / `#C2410C` |
| 22 | 冬日雪 | 冬日 / 冰雪 / 纯净 | `#0F2E45` / `#38BDF8` / `#7DD3FC` |
| 23 | 圣诞 | 圣诞 / 节日 / 温暖 | `#0E2E1F` / `#16A34A` / `#DC2626` |
| 24 | 万圣节 | 万圣 / 鬼怪 / 派对 | `#1A0B14` / `#F97316` / `#A855F7` |
| 25 | 春节 | 春节 / 新年 / 喜庆 | `#991B1B` / `#DC2626` / `#FACC15` |
| 26 | 靛蓝 | 金融 / 法律 / 权威 | `#1E1B4B` / `#4F46E5` / `#6366F1` |
| 27 | 天空蓝 | 亲子 / 教育 / 医疗 | `#0C4A6E` / `#0284C7` / `#38BDF8` |
| 28 | 酒红 | 红酒 / 复古 / 奢华 | `#3D0F1E` / `#9F1239` / `#C0A062` |
| 29 | 鼠尾草 | 极简 / 家居 / 北欧 | `#2F3A30` / `#7C8B6F` / `#9CAF88` |
| 30 | 霓虹赛博 | 赛博朋克 / 游戏 / 电音 | `#0D0A14` / `#D946EF` / `#22D3EE` |
| 31 | 房地产 | 房产 / 家居 / 建筑 | `#241F1A` / `#8A6D3B` / `#5B6B7A` |
| 32 | 餐饮美食 | 餐饮 / 美食 / 食欲 | `#3B1212` / `#E0522D` / `#F59E0B` |
| 33 | 运动健身 | 运动 / 健身 / 活力 | `#1E2A3A` / `#F97316` / `#0EA5E9` |
| 34 | 母婴 | 母婴 / 亲子 / 温柔 | `#3A2230` / `#EC4899` / `#FCD34D` |
| 35 | 宠物 | 宠物 / 萌宠 / 温暖 | `#3B2A18` / `#D97706` / `#92400E` |
| 36 | 金融科技 | 金融 / 科技 / 信赖 | `#0B1F33` / `#0EA5E9` / `#14B8A6` |
| 37 | 教育 | 教育 / 培训 / 成长 | `#16324A` / `#2563EB` / `#F59E0B` |
| 38 | 娱乐影视 | 娱乐 / 影视 / 星光 | `#1A0B2E` / `#D946EF` / `#C9A227` |
| 39 | 汽车 | 汽车 / 工业 / 硬朗 | `#1A1F24` / `#3B82F6` / `#94A3B8` |
| 40 | 暮色紫 | 黄昏 / 氛围 / 情绪 | `#1B1433` / `#8B5CF6` / `#FB923C` |
| 41 | 海洋珊瑚 | 海岛 / 度假 / 热带 | `#0A3A43` / `#06B6D4` / `#FB7185` |
| 42 | 石墨银 | 硬件 / 科技 / 极客 | `#17191C` / `#60A5FA` / `#94A3B8` |
| 43 | 和风 | 日式 / 侘寂 / 极简 | `#26333F` / `#4A6B8A` / `#C73E3A` |
| 44 | 港风 | 港风 / 复古 / 霓虹 | `#2A0E12` / `#E53935` / `#39D98A` |
| 45 | 北欧 | 北欧 / 极简 / 家居 | `#2E2E2C` / `#8B7355` / `#5B7A9D` |
| 46 | 新中式 | 新中式 / 东方 / 雅致 | `#1F2E2B` / `#B03A2E` / `#C9A227` |
| 47 | 敦煌 | 敦煌 / 西域 / 壁画 | `#4A2018` / `#C4703A` / `#2E8B7A` |
| 48 | 双十一 | 电商 / 促销 / 狂欢 | `#A01818` / `#F97316` / `#FACC15` |
| 49 | 世界杯 | 足球 / 体育 / 激情 | `#0E3B2E` / `#16A34A` / `#FACC15` |
| 50 | 音乐节 | 音乐节 / 演出 / 狂欢 | `#2A1030` / `#EC4899` / `#FACC15` |
| 51 | 复古波普 | 波普 / 撞色 / 潮流 | `#1A1A2E` / `#FF3B5C` / `#FFD23F` |
| 52 | 极光 | 极光 / 自然 / 梦幻 | `#0B1D33` / `#34D399` / `#A78BFA` |
| 53 | 太空 | 太空 / 航天 / 深空 | `#0A0A14` / `#6366F1` / `#38BDF8` |
| 54 | 宋韵 | 宋式 / 留白 / 雅致 | `#2A2E33` / `#6B7280` / `#B04A3A` |
...
- `scripts/gen.py` —— 一键生成配图（cover / infographic / quote / compare / steps / stats / timeline / feature / chart / flow，套配色卡+自动排版+装饰底纹）
